...
DEFAULT_SINGLE_OUTPUT = (
    ROOT / "artifacts" / "skin-finish-learned-mid-surface-probe-20260825-v1"
)
DEFAULT_SIX_OUTPUT = (
    ROOT / "artifacts" / "skin-finish-learned-mid-surface-calibration-20260825-v1"
)
FUSION_PARAMETERS = OrderedDict(
    [
        ("broad_amount", 0.65),
        ("mid_amount", 0.85),
        ("fine_split_radius_px", 2),
        ("broad_radius_px", 12),
        ("maximum_broad_component_delta", 0.18),
        ("maximum_mid_component_delta", 0.12),
        ("candidate_rgb_delta_cap", 0.10),
        ("minimum_masked_mean_abs_change", 0.012),
        ("maximum_masked_mean_abs_change", 0.060),
        ("minimum_applied_mid_rms", 0.0035),
        ("minimum_source_fine_cosine", 0.95),
        ("minimum_texture_ratio", 0.88),
        ("maximum_texture_ratio", 1.20),
...
MINIMUM_SFACE_SOURCE_CANDIDATE_COSINE = (
    SFACE_OFFICIAL_COSINE_THRESHOLD + SFACE_ADDITIONAL_SAFETY_MARGIN
)
MINIMUM_SFACE_IMPROVEMENT_OVER_RAW_PROPOSAL = 0.02
MINIMUM_FULL_FRAME_MASKED_MEAN_CHANGE = 0.018


def _aligned_sface_feature(image, detector, recognizer):
    import numpy as np
    ...
    array = image[0].mul(255.0).round().clamp(0, 255).byte().numpy()
    detection = learned_detail._largest_detection(
        _detect_yunet_rgb(detector, np.ascontiguousarray(array))
    )
    return _sface_feature(image[0], detection, recognizer), detection


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Probe one fixed broad-plus-mid GFPGAN skin-surface candidate. The raw proposal's "
            "geometry and fine detail are never pasted. Six-frame work is locked behind the "
            "single-frame mechanical, relative-identity and human-visibility gate."
        )
    )
    ...
    if args.mode == "six" and not args.confirm_single_gate_passed:
        raise RuntimeError(
            ...
            "single-frame candidate passes mechanical, relative-identity and human review gates"
        )
    ...
    from h3_audio_t8_skin_finish_quality_stream_validation.skin_finish_learned_mid_surface import (
    ...
    try:
        ...
        for frame_index, array in zip(indices, arrays, strict=True):
            ...
            if not torch.equal(full_candidate[outside], source_full[outside]):
                raise RuntimeError("full-frame mid-surface candidate changed mask exterior")
            ...
            source_feature, source_aligned_detection = _aligned_sface_feature(
                aligned, aligned_detector, recognizer
            )
            proposal_feature, proposal_detection = _aligned_sface_feature(
                proposal, aligned_detector, recognizer
            )
            candidate_feature, candidate_aligned_detection = _aligned_sface_feature(
                candidate, aligned_detector, recognizer
            )
            raw_proposal_similarity = float(torch.dot(source_feature, proposal_feature))
            aligned_candidate_similarity = float(
                torch.dot(source_feature, candidate_feature)
            )
            ...
            frame_reports.append(
                {
                    ...
                    "sface_source_raw_proposal_cosine": round(
                        raw_proposal_similarity, 8
                    ),
    ...
    aligned_sheet = output / "aligned_source_gfpgan_proposal_mid_surface_candidate.png"
    learned_detail._write_aligned_sheet(
        ...
        OrderedDict(
            [
                ...
                ("broad + mid candidate", aligned_candidate_rows),
            ]
        ),
        indices,
    )
    full_sheet = output / "full_frame_source_vs_mid_surface_candidate.png"
    calibration._write_contact_sheet(
        full_sheet,
        OrderedDict([("source", arrays), ("broad + mid candidate", full_candidate_rows)]),
        records,
        list(indices),
        [PINNED_OILY_SCORES[index] for index in indices],
    )
    report = {
        "schema": "h3_t8_skin_finish_learned_mid_surface_calibration/v1",
        ...
        "model": {
            ...
            "only_broad_and_mid_skin_surface_transferred": True,
        },
        "identity": {
        ...
        "claim_boundary": (
            "Passing one fixed frame means only that this one broad-plus-mid candidate is "
            "visible, bounded and closer to source by the declared SFace signals than raw "
            "GFPGAN. It cannot establish temporal stability or general aesthetic benefit."
        ),
    }
