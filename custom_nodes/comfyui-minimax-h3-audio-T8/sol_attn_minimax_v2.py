...
def _sol_kernel_parameters(kernel):
    """Return an inspected keyword set, or ``None`` for opaque callables.

    Sol-Attn was initially shipped through experimental comfy-kitchen builds
    with ``max_blocks``, ``centroid_tail`` and ``reuse_qkv_memory``.  The
    current public API uses a smaller, different contract.  Inspecting the
    callable keeps this single-file node usable with both generations without
    retrying a CUDA operation after an arbitrary failure.
    """
    key = id(kernel)
    if key not in _SOL_KERNEL_PARAMETERS:
        try:
            parameters = inspect.signature(kernel).parameters.values()
            _SOL_KERNEL_PARAMETERS[key] = frozenset(
                parameter.name for parameter in parameters
                if parameter.kind in (inspect.Parameter.POSITIONAL_OR_KEYWORD,
                                      inspect.Parameter.KEYWORD_ONLY)
            )
        except (TypeError, ValueError):
            _SOL_KERNEL_PARAMETERS[key] = None
    return _SOL_KERNEL_PARAMETERS[key]


def _call_sol_kernel(kernel, q, k, v, *, tau, scale, sink_blocks, sink_q,
                     max_blocks, routed_cap_percent, centroid_tail,
                     reuse_qkv_memory):
    """Call either the legacy experimental or current public Sol API."""
    parameters = _sol_kernel_parameters(kernel)
    kwargs = {
        "tau": tau,
        "scale": scale,
    ...
    if parameters is not None and "max_blocks" in parameters:
        kwargs["max_blocks"] = max_blocks
    elif routed_cap_percent:
        # Current `topk_ratio` is not a rename of max_blocks: it replaces the
        # tau threshold with SLA top-k selection.  Ignoring the old cap is the
        # quality-preserving compatibility choice.
        _log_once(
            ("unsupported", "max_blocks"),
            "this comfy_kitchen Sol API has no max_blocks route cap; "
            "routed_cap_percent is ignored",
        )

    if parameters is not None and "centroid_tail" in parameters:
        kwargs["centroid_tail"] = centroid_tail
    elif parameters is not None and "tail" in parameters:
        # The public API's `tail=False` drops the pooled branch entirely; it is
        # not equivalent to the legacy per-row tail selected by
        # centroid_tail=False.  Keep the quality-preserving pooled tail.
        kwargs["tail"] = True
        if not centroid_tail:
            _log_once(
                ("unsupported", "centroid_tail"),
                "this comfy_kitchen Sol API always uses the centroid pooled "
                "tail; centroid_tail=False is ignored to avoid dropping it",
            )
    elif not centroid_tail:
        _log_once(
            ("unsupported", "centroid_tail"),
            "this comfy_kitchen Sol API has no centroid_tail control; the "
            "requested override is ignored",
        )

    if parameters is not None and "reuse_qkv_memory" in parameters:
        kwargs["reuse_qkv_memory"] = reuse_qkv_memory
    elif reuse_qkv_memory:
        _log_once(
            ("unsupported", "reuse_qkv_memory"),
            "this comfy_kitchen Sol API has no reuse_qkv_memory support; "
            "using its normal output allocation",
        )
...
def _run(q, k, v, heads, skip_reshape, skip_output_reshape, scale,
    ...
    kernel = _ck.sol_attn
    if reuse_qkv_memory:
        # Legacy experimental builds exposed the in-place mode only through
        # the CUDA backend.  Prefer it only when its inspected signature still
        # advertises the capability; current public builds do not.
        try:
            from comfy_kitchen.backends import cuda as _ck_cuda
            cuda_parameters = _sol_kernel_parameters(_ck_cuda.sol_attn)
            if cuda_parameters is not None and "reuse_qkv_memory" in cuda_parameters:
                kernel = _ck_cuda.sol_attn
        except (ImportError, AttributeError):
            pass

    out = _call_sol_kernel(
        kernel, qs, ks, vs,
        tau=tau, scale=scale,
        sink_blocks=sink_blocks, sink_q=sink_q,
        max_blocks=cap, routed_cap_percent=routed_cap_percent,
