...
echo Flow Fully Automatic Run
echo.
echo This will open Flow, click New project, enter the prompt, submit, and try to download the result.
echo Your normal Chrome and ComfyUI can stay open.
echo.
set /p "FLOW_PROMPT=Prompt: "
set "OUTPUT_DIR=%~dp0outputs"
if "%FLOW_PROMPT%"=="" set "FLOW_PROMPT=a woman with red hair"
...
echo Running automation now. No browser clicks should be needed.
call npm start -- --url "%FLOW_URL%" --prompt "%FLOW_PROMPT%" --out "%OUTPUT_DIR%" --connect-cdp "http://127.0.0.1:9222"
