...
const NODE_NAMES = new Set(["VRGDG_FlowBrowserImageEdit", "VRGDG_ChatGPTImagesBrowser"]);
const MAX_IMAGES = 50;
...
function refreshInputs(node) {
  ...
  const count = Math.max(0, Math.min(MAX_IMAGES, Number(countWidget.value ?? 1)));
...
function bindCountChange(node) {
  const countWidget = getWidget(node, "image_count");
  if (!countWidget || countWidget.__vrgdgFlowImageEditBound) return;
  ...
  countWidget.__vrgdgFlowImageEditBound = true;
}

app.registerExtension({
  name: "vrgdg.flowBrowser.dynamicImages",
