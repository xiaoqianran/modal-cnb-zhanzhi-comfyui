...
const NODE_NAME = "VRGDG_MultiReferenceConditioning";
const MAX_IMAGES = 50;
...
function refreshInputs(node) {
  ...
  const count = Math.max(1, Math.min(MAX_IMAGES, Number(countWidget.value ?? 4)));
...
function bindCountChange(node) {
  const countWidget = getWidget(node, "image_count");
  if (!countWidget || countWidget.__vrgdgMultiReferenceBound) return;
  ...
  countWidget.__vrgdgMultiReferenceBound = true;
}
