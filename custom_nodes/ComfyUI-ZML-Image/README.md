...
## 💕简单介绍一下节点：
**这是一个功能丰富的COMFYUI自定义节点，目前节点总数共有160+，各个方面的节点都有，已经不再是仅有图像了。**

**节点对环境的要求很低，不会破坏你的comfyui环境**

`我是按照自己的需求来开发的，我经常使用的节点功能会很完善，我自己都不怎么用的节点，可能会有些bug……节点太多了，一个人顾不过来。如果有bug可以提issues，有时间会尝试修复的，当然你也可以自己尝试修复，修复好后给我提交合并请求即可！`
...
> - #### 合并了 https://github.com/zml-w/ComfyUI-ZML-Image/pull/19 和 https://github.com/zml-w/ComfyUI-ZML-Image/pull/20 提交的bug修改，感谢贡献。
>
> - #### 修复了已知问题。
...
> - #### 合并了 https://github.com/zml-w/ComfyUI-ZML-Image/pull/18 提交的bug修改，感谢贡献。
>
> - #### 添加了[Hanzhihu](https://github.com/Hanzhihu)制作的NovelAI相关节点，待完善。
>
> - #### 修复了已知bug。
>
> - #### 添加了两个比较实用的节点，获取节点的代码和打印python路径，前者可以帮你查看其它节点的源代码，后者可以帮你打印comfyui的python路径（有时候很有用！）。
>
> - #### 添加了图层分离节点，输入透明背景或者纯色背景的图，节点会将不连贯的像素进行分离，比如三视图抠图白底后分离出来为三个单视图。
>
> - #### 更新了预设分辨率V3节点，可以为每个模型制作独立的预设分辨率，这样就不用把sdxl的和其它模型的预设分辨率都挤在一起了。
>
> - #### 添加了通知节点，可以comfyui内部通知和系统通知，也带音频。
>
> - #### 文本输入节点添加了可选的输入接口、添加了批次转列表节点、裁剪图像节点更新了、LLM节点改成了openai库、添加了拆分宫格节点。

> ### 更新日期： **2026.03.29**
>

> - #### 强力LoRA加载器添加了鼠标悬停预览图像，兼容信息文件在LoRA同目录。
>
> - #### 添加了小番茄混淆节点，原作者不太清楚，但应该是开源的（很多地方看到源文件了，做出来是为了更方便适应），如果有问题，可以联系我删除。
>
> - #### 添加了个清理显存和内存的节点。
...
> - #### 文本选择V3节点，锁定模式新增了个“文件夹下拉选择”模式，开启后可以下拉选择文件夹中开启的文本框，类似于那些纯py的预设文本节点。
>
> - #### 新增了一个路径加载LoRA的节点，可以从任意路径的文件夹中加载LoRA，且可以根据索引选择使用哪个LoRA，配合一些整数节点使用就可以做到按照顺序加载LoRA或随机加载LoRA。
>
...
> - #### 修复了些小bug。
>
> - #### 画画节点自适应动画，如果输入图像没有更新，则会缓存上次的自适应动画的尺寸，重复打开时无需重新加载。
>
> - #### 标签化图像V2节点，修复了无法编辑文本块的问题，添加了样式切换的按钮，不再只有缩略图了。
>
...
> - #### 修复了文本图像节点的bug。
>
> - #### 将[《喵咔》](https://civitai.com/user/MIAOKA)的妙妙小工具制作成了筛选提示词V2节点，
>
...
> <img width="1611" height="754" alt="86637389058618596B347A3A32B22F4F" src="https://github.com/user-attachments/assets/9ae8bd04-4f8c-4527-9533-5e02acc9c0d9" />
...
## ✨QQ交流群：[1093764831](https://qm.qq.com/q/exEUdvzrgW)

- **ZML-Image节点的初衷是方便保存图像提示词，也就是节点里经常会出现的关键词“文本块”。**

- **我在群聊里分享了一些包含文本块的图像和小工具以及工作流（都是节点相关的），也欢迎大家一起来分享自己保存的图像文本块。**
...
### 筛选提示词V2使用到了[《喵咔》](https://civitai.com/user/MIAOKA)手工筛选的词库
