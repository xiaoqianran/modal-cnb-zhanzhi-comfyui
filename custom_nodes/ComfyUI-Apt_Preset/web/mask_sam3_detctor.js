...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    const oldCreated = nodeType.prototype.onNodeCreated;
      ...
      const promptWidget = findWidget(this, "pos");
      hideWidget(positiveWidget);
      hideWidget(negativeWidget);
      hideWidget(bboxesJsonWidget);
      hideWidget(promptWidget);

      const advancedWidget = this.addWidget("toggle", "高级模式", false, null);
      hideWidget(advancedWidget);
      const nativeNodeHeight = Math.max(MIN_NODE_HEIGHT, Math.ceil(this.computeSize?.()[1] || MIN_NODE_HEIGHT));
      ...
      promptInput.placeholder = "pos";
      promptInput.value = String(promptWidget?.value || "");
      promptInput.style.width = "100%";
      promptInput.style.height = `${PROMPT_WIDGET_HEIGHT}px`;
      promptInput.style.minHeight = `${PROMPT_WIDGET_HEIGHT}px`;
      promptInput.style.maxHeight = `${PROMPT_WIDGET_HEIGHT}px`;
      promptInput.style.flex = "0 0 auto";
      promptInput.style.boxSizing = "border-box";
      promptInput.style.resize = "none";
      promptInput.style.border = "1px solid #3f3f46";
      promptInput.style.borderRadius = "6px";
      promptInput.style.background = "#303030";
      promptInput.style.color = "#d1d5db";
      ...
      advancedRow.style.justifyContent = "space-between";
      advancedRow.style.height = "28px";
      advancedRow.style.flex = "0 0 28px";
      advancedRow.style.color = "#d1d5db";
      advancedRow.textContent = "高级模式";

      const advancedToggle = document.createElement("input");
      advancedToggle.type = "checkbox";
      advancedToggle.checked = Boolean(advancedWidget.value);
      ...
      visualPanel.style.gap = "6px";
      visualPanel.style.flex = "1";
      visualPanel.style.minHeight = "0";
      visualPanel.style.overflow = "hidden";
      ...
      canvasWrap.style.minHeight = "80px";
      ...
      visualPanel.appendChild(pointInputRow);
      visualPanel.appendChild(canvasWrap);
      visualPanel.appendChild(previewBtn);

      const nodeInstance = this;
      let advancedEnabled = false;
      const COLLAPSED_UI_HEIGHT = PROMPT_WIDGET_HEIGHT + 34;
      const VISUAL_DEFAULT_HEIGHT = 320;
      ...
      uiWidget.computeSize = function (width) {
        return [width, COLLAPSED_UI_HEIGHT];
      };
      const COLLAPSED_NODE_HEIGHT = typeof nodeInstance.computeSize === "function"
        ? nodeInstance.computeSize()[1]
        : nativeNodeHeight + COLLAPSED_UI_HEIGHT;
      const UI_BASE_HEIGHT = COLLAPSED_NODE_HEIGHT + VISUAL_DEFAULT_HEIGHT;
      ...
      uiWidget.computeSize = function (width) {
        if (!advancedEnabled) return [width, COLLAPSED_UI_HEIGHT];
        const targetH = getTargetNodeHeight();
        const extra = Math.max(0, (targetH - UI_BASE_HEIGHT) * 0.95);
        return [width, COLLAPSED_UI_HEIGHT + VISUAL_DEFAULT_HEIGHT + extra];
      };
      uiWidget.computeLayoutSize = function () {
        const minHeight = advancedEnabled
          ? COLLAPSED_UI_HEIGHT + VISUAL_DEFAULT_HEIGHT
          : COLLAPSED_UI_HEIGHT;
        return {
          minHeight,
          maxHeight: advancedEnabled ? undefined : minHeight,
          minWidth: 0,
        };
      };

      const setAdvancedMode = (enabled, resizeNode = true) => {
        advancedEnabled = Boolean(enabled);
        advancedToggle.checked = advancedEnabled;
        visualPanel.style.display = advancedEnabled ? "flex" : "none";
        nodeInstance.min_size = [MIN_NODE_WIDTH, MIN_NODE_HEIGHT];

        if (resizeNode) {
          const height = advancedEnabled ? Math.max(nodeInstance.size[1], UI_BASE_HEIGHT) : COLLAPSED_NODE_HEIGHT;
          nodeInstance.setSize([Math.max(MIN_NODE_WIDTH, nodeInstance.size[0]), height]);
        }
        nodeInstance.graph?.setDirtyCanvas?.(true, true);
        if (advancedEnabled) requestAnimationFrame(() => draw());
      };
      this._samSetAdvancedMode = setAdvancedMode;
      advancedWidget.callback = (value) => {
        setAdvancedMode(Boolean(value));
        nodeInstance.graph?.change?.();
      };
      advancedToggle.addEventListener("change", () => {
        advancedWidget.value = advancedToggle.checked;
        advancedWidget.callback?.(advancedToggle.checked);
      });
      promptInput.addEventListener("input", () => {
        if (!promptWidget) return;
        promptWidget.value = promptInput.value;
      ...
      setAdvancedMode(Boolean(advancedWidget.value));
      updateModeUI();
      draw();
    };
    ...
    nodeType.prototype.onConfigure = function () {
      oldConfigured?.apply(this, arguments);
      setTimeout(() => {
        const advancedWidget = findWidget(this, "高级模式");
        this._samSyncPrompt?.();
        this._samSetAdvancedMode?.(Boolean(advancedWidget?.value));
      }, 0);
    };
    ...
    nodeType.prototype.onRemoved = function () {
      ...
      this._samSetAdvancedMode = null;
      this._samSyncPrompt = null;
