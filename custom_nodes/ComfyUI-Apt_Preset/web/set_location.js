...
app.registerExtension({
    name: "Comfy.Coordinate_fromImage",
    async beforeRegisterNodeDef(nodeType, nodeData, appInstance) {
        if (nodeData.name !== "Coordinate_fromImage") {
            return;
        }
        ...
        nodeType.prototype.onNodeCreated = function() {
            ...
            this.size = [420, 440]; // 只减少节点高度，保持预览框合适
        ...
        nodeType.prototype.onRemoved = function() {
            // 清理定时器
            if (this._imageInputInterval) {
                clearInterval(this._imageInputInterval);
                this._imageInputInterval = null;
            }
        ...
        nodeType.prototype.onConnectionsChange = function(type, index, connected, link_info) {
            const r = originalOnConnectionsChange ? originalOnConnectionsChange.apply(this, arguments) : undefined;
        ...
        nodeType.prototype._createImagePreview = function() {
            ...
            this.CONTAINER_HEIGHT = 320; // 恢复到合适的预览框高度
            
            // 创建一个间距容器，将预览框向下推
            const spacingContainer = document.createElement("div");
            spacingContainer.style.cssText = `width: 100%; height: 10px;`; // 10px的间距
            ...
            placeholder.textContent = "等待图片输入";
            ...
            // 先添加间距容器
            const spacingWidget = this.addDOMWidget("preview_spacing", "customWidget", spacingContainer, {
                serialize: false,
                hideOnZoom: false
            });
            spacingWidget.computeSize = (width) => [width - 20, 10];
        ...
        nodeType.prototype._setupImageWidget = function() {
            const node = this;
            // 监听输入连接变化
            this.onInputAdded = function(slot) {
                if (slot.name === "image") {
                    // 当有图片输入连接时，开始监听数据流
                    node._startImageInputMonitoring();
                }
            };
            
            this.onInputRemoved = function(slot) {
                if (slot.name === "image") {
                    // 当图片输入连接断开时，停止监听并清空预览
                    node._stopImageInputMonitoring();
                    node._clearPreview();
                }
            };
            
            // 检查是否已有连接的输入
            if (this.inputs && this.inputs.find(input => input.name === "image" && input.link)) {
                this._startImageInputMonitoring();
            }
            ...
            setTimeout(() => {
                if (this.widgets) {
                    this.widgets.forEach(w => {
                        if (w.type === "IMAGEUPLOAD" || w.name === "upload" || w.name === "image") {
                            if (w.element) {
        ...
        nodeType.prototype._startImageInputMonitoring = function() {
            const node = this;
            if (this._imageInputInterval) {
                clearInterval(this._imageInputInterval);
            }
            ...
            // 定期检查输入的图片数据
            this._imageInputInterval = setInterval(() => {
                node._checkImageInput();
            }, 500);
        };
        
        nodeType.prototype._stopImageInputMonitoring = function() {
            if (this._imageInputInterval) {
                clearInterval(this._imageInputInterval);
                this._imageInputInterval = null;
            }
        ...
        nodeType.prototype.loadInputImage_internal = function() {
            ...
            if (sourceNode.imgs && sourceNode.imgs.length > 0) {
                ...
                if (img && img.src !== this.currentImageSrc) {
                    this._loadImageFromTensor(img);
                }
            }
        ...
        nodeType.prototype._checkImageInput = function() {
            // 保留此方法以兼容旧的调用
            this.loadInputImage_internal();
        };
        
        nodeType.prototype._loadImageFromTensor = function(img) {
            ...
            this.imgElement.onload = () => {
                console.log("Coordinate_fromImage 图片加载成功");
                // 如果没有坐标点，创建默认的中心点
                if (this.pointsData.length === 0) {
                    this.pointsData = [{
                        x: 0.5,
        ...
        nodeType.prototype._clearPreview = function() {
            this.currentImageSrc = null;
            this.imgElement.src = "";
            this.imgElement.style.display = "none";
            this.placeholder.style.display = "block";
            this.placeholder.textContent = "等待图片输入";
            this.imageUrl = null;
            this._clearAllMarkers();
            this.setDirtyCanvas(true);
        };
        nodeType.prototype._loadImage = function(filename) {
            // 保留此方法以兼容旧的工作流，但现在主要用于文件上传的情况
            if (!filename) return;
        ...
        nodeType.prototype._updatePointsWidget = function() {
            ...
            if (pointsWidget) {
                const coordinatesJson = JSON.stringify(this.pointsData);
                pointsWidget.value = coordinatesJson;
                console.log("Coordinate_fromImage 更新坐标数据:", coordinatesJson); // 调试输出
            }
            // 强制触发节点重新执行
            if (this.graph) {
                this.graph.change();
                ...
                setTimeout(() => {
                    if (this.outputs && this.outputs.length > 0) {
                        this.setOutputData(1, JSON.stringify(this.pointsData));
                    }
                }, 50);
            }
        };
