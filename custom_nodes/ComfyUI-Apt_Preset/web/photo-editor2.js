...
class PhotopeaEditorModal extends ComfyDialog {
  ...
  show() {
    ...
    this.iframeElement = $el("iframe", {
      src: `https://www.tugaigai.com/online_ps/`,
...
app.registerExtension({
  ...
  init(appInstance) {
    ...
    // ClipspaceDialog 在新版前端中不再是公开 API，尝试通过 app 获取
    const clipspaceDialog = app.clipspaceDialog || window.comfyAPI?.clipspace?.ClipspaceDialog;
    if (clipspaceDialog?.registerButton) {
      clipspaceDialog.registerButton("在线PS", isValidContext, showModal);
    }
  },
