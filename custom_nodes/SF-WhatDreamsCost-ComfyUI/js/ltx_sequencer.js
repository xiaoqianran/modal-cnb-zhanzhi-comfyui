...
// Global registry to track all SFLTXSequencer nodes across all subgraphs
window._SFLTXSequencerGlobalNodes = window._SFLTXSequencerGlobalNodes || new Set();
...
function syncFullStateAcrossNodes(sourceNode) {
    if (!window._SFLTXSequencerGlobalNodes) return;
    
    for (const targetNode of window._SFLTXSequencerGlobalNodes) {
...
app.registerExtension({
    name: "Comfy.SFLTXSequencer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "SFLTXSequencer") return;

        // Register this node instance globally
        window._SFLTXSequencerGlobalNodes.add(node);
        ...
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "SFMultiImageLoader") return originNode;
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode?.comfyClass === "SFMultiImageLoader") return innerNode;
                    } catch (e) {}
                }
                return null;
            }
            ...
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "SFMultiImageLoader") multiImageLoaders.push(n);
        ...
        node.onRemoved = function() {
            window._SFLTXSequencerGlobalNodes.delete(node);
            clearInterval(pollInterval);
