...
// Global registry to track all SFLTXKeyframer nodes across all subgraphs
window._SFLTXKeyframerGlobalNodes = window._SFLTXKeyframerGlobalNodes || new Set();
...
// Finds all other SFLTXKeyframer nodes globally and mirrors the value to them
function syncWidgetAcrossNodes(sourceNode, widgetName, value) {
    if (!window._SFLTXKeyframerGlobalNodes) return;
    
    for (const targetNode of window._SFLTXKeyframerGlobalNodes) {
        // Target all OTHER SFLTXKeyframer nodes by direct object reference
...
app.registerExtension({
    name: "Comfy.SFLTXKeyframer.DynamicInputs",
    async nodeCreated(node) {
        if (node.comfyClass !== "SFLTXKeyframer") return;

        // Register this node instance globally
        window._SFLTXKeyframerGlobalNodes.add(node);
        ...
        // Helper: read image count from a connected SFMultiImageLoader node
        function readSourceImageCount(self) {
            ...
            function traceUpstream(graph, linkId, visited = new Set()) {
                ...
                if (originNode.comfyClass === "SFMultiImageLoader") {
                    return originNode;
                }
                ...
                if (typeof originNode.getInnerNode === "function") {
                    try {
                        ...
                        if (innerNode && innerNode.comfyClass === "SFMultiImageLoader") {
                            return innerNode;
                        }
                    } catch (e) {
            ...
            // If there is EXACTLY ONE SFMultiImageLoader in the workspace, safely assume that's the one.
            let multiImageLoaders = [];
            function findAllLoaders(nodes) {
                if (!nodes) return;
                for (let n of nodes) {
                    if (n.comfyClass === "SFMultiImageLoader") {
        ...
        node.onRemoved = function() {
            ...
            window._SFLTXKeyframerGlobalNodes.delete(node);
