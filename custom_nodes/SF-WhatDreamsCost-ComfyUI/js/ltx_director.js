...
// --- 脚本识别：正面词 + 第X个镜头 + 位置 + X秒 ---
// 宫格方位词 -> grid_index（按布局，行优先：从左到右、从上到下）
// 4/6/9 宫格必须严格对应，否则图片与分镜提示词错位。
const GRID_POSITION_MAPS = {
  "2,2": {
    "左上": 0, "上左": 0,
    "右上": 1, "上右": 1,
    "左下": 2, "下左": 2,
...
const GRID_POSITION_WORDS = (() => {
  const s = new Set();
  for (const k in GRID_POSITION_MAPS) for (const w in GRID_POSITION_MAPS[k]) s.add(w);
  return Array.from(s).sort((a, b) => b.length - a.length); // 长词优先，避免子串误匹配
})();

function positionToIndexJS(word, cols, rows) {
  if (!word) return null;
  const mp = GRID_POSITION_MAPS[`${cols},${rows}`] || GRID_POSITION_MAPS["2,2"];
  const idx = mp[word];
  if (idx == null || idx < 0 || idx >= cols * rows) return null;
  return idx;
}
const SHOT_SIZES = ["大特写", "特写", "中特写", "大近景", "中近景", "近景",
  "中景", "中远景", "远景", "大远景", "大全景", "全景"];

function stripGlobalLabelJS(text) {
  let t = String(text || "").replace(/^\s*(?:正面词|正面|全局提示词|全局词|全局|总提示词|总词|总览词|概述|总览|视频提示词|视频词)\s*[:：]\s*/, "");
  return t.trim();
}

function parseSegmentBodyJS(body) {
  const b = body || "";
  let position = null;
  for (const word of GRID_POSITION_WORDS) {
    if (b.indexOf(word) !== -1) { position = word; break; }
  }
  let durationSeconds = null;
  // 不能用 \b：JS 的 \b 是 ASCII-only，中文"秒"不算单词字符，会导致 "3秒，" 匹配失败；
  // 改用否定前瞻 (?![0-9a-zA-Z])，单位后只要不紧跟字母/数字即视为有效时长。
  const dm = b.match(/(\d+(?:\.\d+)?)\s*(?:秒钟|秒|s)(?![0-9a-zA-Z])/i);
  if (dm) durationSeconds = parseFloat(dm[1]);
  let prompt = b;
  // 剔除方位词(用于宫格映射)与时长"X秒"(已由 durationSeconds 驱动时间轴, 内联冗余); 保留景别(中景/近景…)
  for (const word of GRID_POSITION_WORDS) prompt = prompt.split(word).join("");
...
function recognizeScript(rawText) {
  const text = stripFence(rawText || "");
  if (!text.trim()) return { globalPrompt: "", segments: [] };
  const anchorRe = /第\s*([0-9]+|[一二三四五六七八九十]+)\s*个?\s*(?:镜头|镜|画面|分镜|shot)/gi;
  const anchors = [...text.matchAll(anchorRe)];
  if (!anchors.length) {
    return { globalPrompt: stripGlobalLabelJS(text), segments: [] };
  }
  const globalPrompt = stripGlobalLabelJS(text.slice(0, anchors[0].index));
  const segments = [];
  for (let i = 0; i < anchors.length; i++) {
    const start = anchors[i].index + anchors[i][0].length;
    const end = i + 1 < anchors.length ? anchors[i + 1].index : text.length;
    segments.push(parseSegmentBodyJS(text.slice(start, end)));
  }
  return { globalPrompt, segments };
}

function gridDimsFromLayout(layout) {
  const m = String(layout || "2x2").toLowerCase().match(/(\d+)\s*[x×]\s*(\d+)/);
  if (m) return { cols: parseInt(m[1], 10), rows: parseInt(m[2], 10) };
  return { cols: 2, rows: 2 };
}

// djb2 哈希（按 UTF-16 码元），与后端 _grid_text_signature 完全一致。
// 前端用脚本构建时间线时把签名写入 timeline._grid_text_sig；
// 后端执行时比对签名，不一致（如万象回填了新脚本）才重建时间线，一致则尊重用户手动调整。
function gridTextSignature(text) {
  let h = 5381;
  const s = String(text || "");
...
function buildCellMap(segments, count, cols, rows) {
  const map = new Array(count).fill(null);
  const placed = new Set();
  const queue = [];
  for (const seg of segments) {
    let idx = seg.position ? positionToIndexJS(seg.position, cols, rows) : null;
    if (idx != null && idx < count && !placed.has(idx)) { placed.add(idx); map[idx] = seg; }
    else queue.push(seg);
  }
  let qi = 0;
  for (let i = 0; i < count; i++) {
    if (!map[i] && qi < queue.length) map[i] = queue[qi++];
  }
  for (let i = 0; i < count; i++) {
    if (!map[i]) map[i] = { prompt: "", position: null, durationSeconds: null };
  }
  return map;
}
...
function parseInitial(jsonStr) {
  ...
  try {
    if (jsonStr) {
      ...
      if (p._grid_text_sig !== undefined) parsed._grid_text_sig = p._grid_text_sig;
...
class TimelineEditor {
  constructor(node, container, domWidget) {
    ...
    console.log("[SFLTXDirector debug] Constructor: timelineDataWidget value:", this.timelineDataWidget?.value);
    ...
    console.log("[SFLTXDirector debug] Constructor: parsed timeline:", JSON.stringify(this.timeline));
  ...
  async _getOrExtractAudio(seg) {
    ...
    if (this._audioExtractionPromises.has(fileKey)) {
      try {
        ...
        console.warn("[SFLTXDirector] Awaiting shared server audio extract promise failed:", err);
      }
      return;
    }
    ...
    try {
      ...
      console.warn("[SFLTXDirector] Server audio check/extract failed:", err);
    } finally {
  ...
  createDOM() {
    ...
    recognizeBtn.className = "pr-btn";
    recognizeBtn.innerHTML = `${ICONS.text} 识别脚本`;
    recognizeBtn.title = "从'宫格分镜文本'识别：全局提示词(剥离正面词标签) + 各宫格分镜 + 位置(4宫格:左上/右上/左下/右下；6宫格增加中上/中下；9宫格增加左中/右中/中心)严格对应宫格图片 + 时长(X秒)";
    recognizeBtn.addEventListener("click", () => this.recognizeAndApplyScript());
    this.recognizeBtn = recognizeBtn;
    ...
    actionGroup.appendChild(recognizeBtn);
  ...
  syncWidgetsAndUI() {
    console.log("[SFLTXDirector debug] syncWidgetsAndUI() called.");
  ...
  async autoFillFromGrid(onlyIfEmpty = false, source = null, sourceKey = "") {
    ...
    const recognized = recognizeScript(this.getCurrentGridText());
    const cellMap = buildCellMap(recognized.segments, count, source.cols, source.rows);
    const even = evenGridLengths(this.getDurationFrames(), count);
    let cursor = 0;
    const segments = [];
    for (let idx = 0; idx < count; idx++) {
      const cell = cellMap[idx];
      const preview = this.cropGridPreview(img, idx, source.cols, source.rows);
      const length = cell.durationSeconds ? Math.max(1, Math.round(cell.durationSeconds * fr)) : (even[idx] || 1);
      segments.push({
    ...
    // 识别到各分镜时长时，自动把总时长撑到能容纳整段脚本（只增不减）
    if (cursor > this.getDurationFrames()) this.growTimelineIfNeeded(cursor);
    if (recognized.globalPrompt) this.syncGlobalPrompt(recognized.globalPrompt);
    // 记录本次时间线由哪段 grid_text 生成，供后端签名比对（不一致才重建）
    this.timeline._grid_text_sig = gridTextSignature(this.getCurrentGridText());
  ...
  syncGridPromptsFromText() {
    ...
    const recognized = recognizeScript(this.getCurrentGridText());
    if (recognized.globalPrompt) this.syncGlobalPrompt(recognized.globalPrompt);
    // 同步提示词来源签名（时间线由当前 grid_text 驱动）
    this.timeline._grid_text_sig = gridTextSignature(this.getCurrentGridText());
    const segs = this.timeline.segments || [];
    if (!segs.length) return false;
    const { cols, rows } = gridDimsFromLayout(this.gridLayoutWidget?.value);
    const cellMap = buildCellMap(recognized.segments, segs.length, cols, rows);
    const fr = this.getFrameRate();
    segs.forEach((seg, idx) => {
      const cell = cellMap[idx];
      if (cell && cell.prompt) seg.prompt = cell.prompt;
      if (cell && cell.durationSeconds) seg.length = Math.max(1, Math.round(cell.durationSeconds * fr));
    });
    this.updateUIFromSelection();
    this.commitChanges(true);
    this.render();
  ...
  recognizeAndApplyScript() {
    const script = this.getCurrentGridText();
    if (!script || !script.trim()) {
      if (window.app && window.app.ui && window.app.ui.dialog) {
        window.app.ui.dialog("请先在'宫格分镜文本'中粘贴分镜脚本");
      }
      return false;
    }
    const recognized = recognizeScript(script);
    if (recognized.globalPrompt) this.syncGlobalPrompt(recognized.globalPrompt);
    ...
    const source = getGridSource(this.node);
    if (isGridModeEnabled(this.node) && source && source.url) {
      // 有宫格图像：走自动填充（已支持识别位置/时长）
      this.autoFillFromGrid(false);
      return true;
    }
    // 无宫格图像：仅按识别结果生成文本分段（带位置与时长）
    const count = recognized.segments.length;
    const fr = this.getFrameRate();
    const even = evenGridLengths(this.getDurationFrames(), count);
    const segments = [];
    let cursor = 0;
    recognized.segments.forEach((seg, idx) => {
      const length = seg.durationSeconds ? Math.max(1, Math.round(seg.durationSeconds * fr)) : (even[idx] || 1);
      segments.push({
        id: Date.now().toString() + Math.random().toString(36).substr(2, 5),
        start: cursor,
        length,
    ...
    if (cursor > this.getDurationFrames()) this.growTimelineIfNeeded(cursor);
    // 记录本次时间线由哪段 grid_text 生成，供后端签名比对（不一致才重建）
    this.timeline._grid_text_sig = gridTextSignature(script);
    this.selectionType = "image";
    this.selectedIndex = segments.length ? 0 : -1;
    this.updateUIFromSelection();
    this.commitChanges(true);
    ...
    try {
      ...
      if (checkResp.status === 200) {
        ...
        if (checkResult.exists) {
          console.log(`[SFLTXDirector] File already exists: ${checkResult.name}. Reusing existing file.`);
          return checkResult.name;
        }
      }
    } catch (e) {
      console.warn("[SFLTXDirector] Failed to check for existing file, proceeding with upload", e);
    }
        ...
        try {
          ...
          vid.onloadeddata = async () => {
            ...
            vid.onseeked = () => {
              ...
              if (IS_LARGE_FILE) {
                console.log(`[SFLTXDirector] Large file detected (${(file.size / 1024 / 1024).toFixed(1)} MB). Offloading audio extraction to server.`);
              } else {
              ...
              this._uploadVideoFile(file).then(filePath => {
                  ...
                  api.fetchApi(`/cs_ltx_director_get_audio?filename=${encodeURIComponent(filePath)}`)
                    ...
                    .then(res => {
                      if (res.audio_file && res.peaks) {
                        ...
                        // Fallback
                        if (IS_LARGE_FILE) {
                          console.warn("[SFLTXDirector] Server audio extraction failed for large file, skipping.");
                          for (let s of this.timeline.audioSegments) {
                            if (s.audioFile === filePath || s._blobUrl === blobUrl) {
                              s._decoding = false;
                            }
                    ...
                    .catch(err => {
                      console.error("[SFLTXDirector] Server audio extraction query failed:", err);
                      for (let s of this.timeline.audioSegments) {
                        if (s.audioFile === filePath || s._blobUrl === blobUrl) {
                          s._decoding = false;
                        }
                ...
                console.error("[SFLTXDirector] Background video upload failed", err);
    ...
    for (let file of files) {
      ...
      await new Promise(async (resolve) => {
        try {
          ...
          vid.onloadeddata = () => {
            ...
            vid.onseeked = () => {
              ...
              this._uploadVideoFile(file).then(filePath => {
                ...
                console.error("[SFLTXDirector] Background motion video upload failed", err);
          ...
          console.error("[SFLTXDirector] Motion video processing failed", err);
          resolve();
        }
      });
    }
  ...
  syncGlobalPrompt(val) {
    // 兜底剥离"正面词："等引导标签：万象等外部回填常把整段脚本（含"正面词："开头）
    // 写进全局提示词，这里统一在写入 UI/时间线前去掉标签，避免"正面词"字样泄漏显示。
    val = stripGlobalLabelJS(val);
    if (this.node.properties) {
        ...
        if (isLiveActive && videoEl && videoEl.readyState >= 2) {
          ...
          if (drawSource && seg.type !== "ghost") {
            ...
            if (natW > 0) {
              ...
              // Clip to segment bounds so images don't bleed into adjacent segments
              this.ctx.save();
              this.ctx.beginPath();
              ...
              if (seg.type === "image") {
                // 宫格/静帧图片：按图片自身比例裁剪填充（object-fit: cover），
                // 不能为了塞满时间轴分段而拉伸变形。
  ...
  commitChanges(skipRender = false) {
    ...
    const toSave = {
      ...
      _grid_text_sig: this.timeline._grid_text_sig || "",
    ...
    console.log("[SFLTXDirector debug] commitChanges: saving timelineDataWidget value:", jsonStr);
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "SFLTXDirector") {
      ...
      nodeType.prototype.onConfigure = function (info) {
        ...
        console.log("[SFLTXDirector debug] onConfigure called. info.widgets_values:", info.widgets_values ? JSON.stringify(info.widgets_values) : "undefined");
        ...
        if (info.properties && info.properties.has_serialized_properties) {
          console.log("[SFLTXDirector debug] Restoring widgets from properties");
          if (this.widgets) {
            for (const w of this.widgets) {
          ...
          console.log("[SFLTXDirector debug] Restoring widgets via fallback name-based schema mapping");
          const SCHEMA_19 = [
        ...
        setTimeout(() => {
          if (this._timelineEditor) {
            console.log("[SFLTXDirector debug] setTimeout sync block called.");
            console.log("[SFLTXDirector debug] setTimeout: timelineDataWidget value:", this._timelineEditor.timelineDataWidget?.value);
            const tl = parseInitial(this._timelineEditor.timelineDataWidget?.value);
            console.log("[SFLTXDirector debug] setTimeout: parsed timeline:", JSON.stringify(tl));
