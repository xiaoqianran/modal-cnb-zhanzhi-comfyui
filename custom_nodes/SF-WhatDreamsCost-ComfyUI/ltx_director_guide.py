...
def _snap_latent_to_downscale(latent_image, noise_mask, downscale_factor, method):
    ...
    log.warning(
        "[SFLTXDirectorGuide] Auto-snapping latent grid from %sx%s to %sx%s for IC-LoRA downscale factor %s.",
...
def _load_lora_model_only(model, ic_lora_name, strength_model):
    ...
    try:
        ...
        log.warning(
            "[SFLTXDirectorGuide] Could not read reference_downscale_factor from %s, using 1.0.",
            ic_lora_name,
        )
...
def _load_motion_video_frames(video_file, trim_start_frames, length_frames, director_fps, resample_mode="nearest"):
    ...
    log.info(f"[SFLTXDirectorGuide] Loading video frames from {path}. start_s: {start_s:.3f}, dur_s: {dur_s:.3f}, end_s: {end_s}")
    ...
    if start_s > 0:
        try:
            ...
            log.info(f"[SFLTXDirectorGuide] PyAV seeked stream to pts={seek_pts} (approx {max(0, start_s - 0.5):.3f}s)")
        except Exception as seek_err:
            log.warning(f"[SFLTXDirectorGuide] Seek failed: {seek_err}, decoding from beginning.")
...
class SFLTXDirectorGuide:
    ...
    def execute(cls, positive, negative, vae, latent, guide_data, motion_guide_data=None, model=None, ic_lora_name="None", ic_lora_strength=1.0, scale_by=1.0, upscale_method="bicubic", image_attention_strength=1.0, crop="center", auto_snap_ic_grid=True, use_tiled_encode=False, tile_size=256, tile_overlap=64, retake_mode=False):
        ...
        print(f"[SFLTXDirectorGuide] execute started. motion_segments: {len(motion_segments)}, image_guides: {image_guides_count}, ic_lora_name: {ic_lora_name}, model connected: {model is not None}, retake_mode: {retake_mode}")
        ...
        # -----------------------------------------------------------------------
        if is_retake_active:
            print(f"[SFLTXDirectorGuide] Retake Mode active. Preserving base latent, masking selected regions. is_empty_latent: {is_empty_latent}")
            ...
            if not is_empty_latent and l_start == 0 and l_end >= latent_length:
                ...
                print("[SFLTXDirectorGuide] Stage 2: Retake region covers the entire generated range. Skipping base video loading and VAE encoding.")
            ...
            if video_file and need_base_video:
                try:
                    print(f"[SFLTXDirectorGuide] Loading and encoding base video file: {video_file} starting at frame {start_frame} for length {ltxv_length} at resolution {target_width}x{target_height}")
                    ...
                    if is_empty_latent:
                        ...
                        print(f"[SFLTXDirectorGuide] Stage 2: Copying high-resolution base latent for preserved regions (0-{l_start} and {l_end}-{paste_len})")
                        if l_start > 0:
                            latent_image[:, :, :l_start] = base_latent[:, :, :l_start]
                        if l_end < paste_len:
                            latent_image[:, :, l_end:paste_len] = base_latent[:, :, l_end:paste_len]
                except Exception as e:
                    print(f"[SFLTXDirectorGuide] Failed to load/encode base video: {e}. Falling back to input latent.")
            ...
            print(f"[SFLTXDirectorGuide] noise_mask slice: {noise_mask[0, 0, :, 0, 0].tolist()}")
        ...
        if len(images) > 0 or len(segments) > 0:
            print(f"[SFLTXDirectorGuide] Using Appended Keyframe Guidance. is_lora_active: {is_lora_active}")
            ...
            print("[SFLTXDirectorGuide] No timeline guides present. Passing through.")
