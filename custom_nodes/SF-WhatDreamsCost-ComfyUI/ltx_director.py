...
# Custom socket type shared with SFLTXSequencer
...
async def ltx_director_check_file(request):
    ...
    try:
        ...
        log.warning(f"[SFLTXDirector] Error listing input directory: {e}")
...
def extract_audio_from_video(video_path):
    ...
    try:
        ...
        print(f"[SFLTXDirector] Server audio extraction failed: {e}")
        return None, None
...
def get_audio_peaks(audio_path):
    ...
    try:
        ...
        print(f"[SFLTXDirector] Failed to get audio peaks via PyAV: {e}")
        return None
...
async def ltx_director_get_audio(request):
    ...
    if is_audio:
        ...
        try:
            ...
            print(f"[SFLTXDirector] Failed to get audio peaks for audio file: {e}")
    ...
    try:
        ...
        print(f"[SFLTXDirector] Error extracting audio: {e}")
...
async def ltx_director_open_folder(request):
    ...
    try:
        ...
        print(f"[SFLTXDirector] Failed to open workspace folder: {e}")
...
async def ltx_director_upload_chunk(request):
    ...
    if chunk_index == total_chunks - 1:
        ...
        try:
            ...
            print(f"[SFLTXDirector] Error in final chunk audio extraction: {e}")
...
# --- Script recognition (正面词 + 第X个镜头 + 位置 + X秒) ---
# 宫格方位词 -> grid_index（按布局，行优先：从左到右、从上到下）
# 4/6/9 宫格的方位词到格位索引必须严格对应，否则图片与分镜提示词错位。
_GRID_POSITION_MAPS = {
    (2, 2): {
        "左上": 0, "上左": 0,
        "右上": 1, "上右": 1,
...
# 所有布局出现过的方位词，用于从分镜正文里剔除（避免污染提示词）
_GRID_POSITION_WORDS = sorted(
    {w for _m in _GRID_POSITION_MAPS.values() for w in _m.keys()},
    key=len, reverse=True,
)


def _position_to_index(word: str | None, cols: int, rows: int):
    """把方位词解析成当前布局下的 grid_index；解析不出或越界返回 None。"""
    if not word:
        return None
...
_G_SHOT_SIZES = ["大特写", "特写", "中特写", "大近景", "中近景", "近景",
                 "中景", "中远景", "远景", "大远景", "大全景", "全景"]


def _strip_global_label(text: str) -> str:
    # 去掉 "正面词：" / "全局提示词：" / "视频提示词：" 之类的引导标签，只保留真正的全局提示词
    t = re.sub(r"^\s*(?:正面词|正面|全局提示词|全局词|全局|总提示词|总词|总览词|概述|总览|视频提示词|视频词)\s*[:：]\s*",
               "", text or "", flags=re.I)
    return t.strip()


def _parse_segment_body(body: str, cols: int = 2, rows: int = 2) -> dict:
    b = body or ""
    position = None
    for word in _GRID_POSITION_WORDS:
        if word in b:
            ...
            if idx is not None:
                position = idx
                break
            # 词能匹配但不在当前布局（如 4宫格里写"左中"），继续找别的词
    dur = None
    # 注意：不能用 \b 作边界——JS 的 \b 是 ASCII-only，中文"秒"不算单词字符，
    # 会导致 "3秒，" 匹配失败；这里用否定前瞻 (?![0-9a-zA-Z]) 保证单位后不紧跟字母/数字即可。
    m = re.search(r"(\d+(?:\.\d+)?)\s*(?:秒钟|秒|s)(?![0-9a-zA-Z])", b, flags=re.I)
    if m:
        dur = float(m.group(1))
    prompt = b
    # 剔除方位词(用于宫格映射)与时长"X秒"(已由 duration_seconds 驱动时间轴, 内联冗余); 保留景别
    for word in _GRID_POSITION_WORDS:
        prompt = prompt.replace(word, "")
    prompt = re.sub(r"\d+(?:\.\d+)?\s*(?:秒钟|秒|s)(?![0-9a-zA-Z])", "", prompt, flags=re.I)
    # 把被剔除词留下的连续标点折叠成单个逗号，避免"中景，，李强"
...
def _recognize_script(text: str, frame_rate: float = 24, cols: int = 2, rows: int = 2):
    """把混合脚本拆成 (global_prompt, segments)。

    期望格式示例：
        正面词：电影感真人摄影，...，无文字无水印无边框。
        第一个镜头，左上，中景，2秒，李强穿蓝色旧T恤坐在单人床边...
        第二个镜头，右上，近景，5秒，李强的手指移向彩票...
    - "正面词：" 等引导标签之后的内容作为全局提示词（不读取标签本身）
    - 按 "第X个镜头" 切分各宫格分镜
    - 每段自动识别位置与时长(X秒)，位置按 cols×rows 布局换算成 grid_index，
      与宫格图片读取顺序（从左到右、从上到下）严格对应，4/6/9 宫格一致。
    """
    text = _strip_fence(text or "")
    if not text.strip():
        return "", []

    anchor_re = re.compile(r"第\s*([0-9]+|[一二三四五六七八九十]+)\s*个?\s*(?:镜头|镜|画面|分镜|shot)", re.I)
    anchors = list(anchor_re.finditer(text))
    if not anchors:
        return _strip_global_label(text), []

    global_prompt = _strip_global_label(text[: anchors[0].start()])
    segments = []
    for i, a in enumerate(anchors):
        start = a.end()
        end = anchors[i + 1].start() if i + 1 < len(anchors) else len(text)
        segments.append(_parse_segment_body(text[start:end], cols, rows))
...
def _build_grid_timeline_if_empty(tdata: dict, grid_text: str, duration_frames: int, layout: str, frame_rate: float = 24) -> dict:
    if tdata.get("segments"):
        return tdata
    cols, rows = _grid_dims_from_layout(layout)
    count = cols * rows
    global_prompt, segments = _recognize_script(grid_text, frame_rate, cols, rows)
    if global_prompt:
        tdata["global_prompt"] = global_prompt
        if not tdata.get("retake_global_prompt"):
            tdata["retake_global_prompt"] = ""

    # 先按识别到的位置落格，未识别位置或冲突的顺次补到空位
    placed = {}
    queue = []
    for seg in segments:
        pos = seg.get("position")
        if pos is not None and 0 <= pos < count and pos not in placed:
            placed[pos] = seg
        else:
            queue.append(seg)

    cell_prompts = [None] * count
    cell_durations = [None] * count
    for idx in range(count):
        if idx in placed:
            cell_prompts[idx] = placed[idx].get("prompt", "")
            cell_durations[idx] = placed[idx].get("duration_seconds")

    qi = 0
    for idx in range(count):
        if cell_prompts[idx] is None and qi < len(queue):
            cell_prompts[idx] = queue[qi].get("prompt", "")
            cell_durations[idx] = queue[qi].get("duration_seconds")
            qi += 1

    # 时长：识别到 X秒 的按 frame_rate 换算成帧；未识别的平分剩余时长
    if any(d for d in cell_durations):
        lengths = [max(1, int(round(d * frame_rate))) if d else None for d in cell_durations]
        none_idx = [i for i, d in enumerate(cell_durations) if d is None]
        allocated = sum(l for l in lengths if l)
        remainder = max(0, int(duration_frames) - allocated)
        if none_idx:
            if remainder > 0:
                even = _even_grid_lengths(remainder, len(none_idx))
                for j, i in enumerate(none_idx):
                    lengths[i] = even[j]
            else:
                base = max(1, int(duration_frames) // len(none_idx))
                for i in none_idx:
                    lengths[i] = base
    ...
    out = []
    for idx in range(count):
        prompt = cell_prompts[idx] or ""
        length = int(lengths[idx]) if lengths and idx < len(lengths) else max(1, int(duration_frames) // count)
        out.append({
...
def _grid_text_signature(text: str) -> str:
    """djb2 哈希（按 UTF-16 码元计算，与前端 JS gridTextSignature 完全一致）。

    前端在"识别脚本/宫格自动填充"时把该签名写进 timeline_data；
    后端执行时若发现签名与当前 grid_text 不符（典型场景：万象回填了新脚本，
    但工作流里保存的 timeline_data 还是旧内容），就按新脚本重建时间线，
    避免视频沿用上一版过期提示词——即"视频提示词没传入"的根因。
    """
    h = 5381
...
def _rebuild_grid_from_script(tdata: dict, global_prompt: str, segments: list,
                              cols: int, rows: int, duration_frames: int,
                              frame_rate: float, sig: str = ""):
    """按识别出的脚本重建宫格时间线，并同步 local_prompts / segment_lengths 字符串。

    返回 (tdata, local_prompts_str, segment_lengths_str)。
    落格规则与 _build_grid_timeline_if_empty 一致：识别到方位的按布局落格，
    未识别/冲突的顺次补空位；时长识别到 X秒 按 frame_rate 换算，未识别的平分剩余时长。
    """
    count = cols * rows
    placed = {}
    queue = []
    ...
    qi = 0
    for idx in range(count):
        if not cell_prompts[idx] and qi < len(queue):
            cell_prompts[idx] = queue[qi].get("prompt", "")
            cell_durations[idx] = queue[qi].get("duration_seconds")
            qi += 1
    ...
    if global_prompt:
        tdata["global_prompt"] = global_prompt
        tdata.setdefault("retake_global_prompt", "")
    if sig:
        tdata["_grid_text_sig"] = sig
    local_prompts_str = " | ".join(cell_prompts[idx] or "" for idx in range(count))
    segment_lengths_str = ",".join(str(int(lengths[idx])) for idx in range(count))
    return tdata, local_prompts_str, segment_lengths_str
...
class SFLTXDirector(io.ComfyNode):
    ...
    def execute(cls, model, clip, start_second, end_second, duration_seconds, start_frame, end_frame, duration_frames,
        ...
        try:
            ...
            log.error(f"[SFLTXDirector] execute timeline_data parse error: {e}")
            tdata = {}
        ...
        if bool(grid_mode) and grid_images is not None:
            _cols, _rows = _grid_dims_from_layout(grid_layout)
            _rg, _rs = ("", [])
            if (grid_text or "").strip():
                _rg, _rs = _recognize_script(grid_text, frame_rate, _cols, _rows)
            if _rs:
                # 脚本可识别：签名比对——timeline 若不是由当前 grid_text 生成
                # （万象回填新脚本后工作流里的 timeline_data 仍是旧内容），
                # 按新脚本整体重建 segments/全局提示词/分段提示词/分段长度；
                # 签名一致则尊重前端时间线（保留用户手动调整）。
                _sig = _grid_text_signature(grid_text)
                if tdata.get("_grid_text_sig") != _sig:
                    log.info("[SFLTXDirector] grid_text 与 timeline 签名不一致，按当前脚本重建宫格时间线")
                    tdata, local_prompts, segment_lengths = _rebuild_grid_from_script(
                        tdata, _rg, _rs, _cols, _rows, duration_frames, frame_rate, _sig)
                    timeline_data = json.dumps(tdata, ensure_ascii=False)
            else:
                # 无可识别脚本：维持旧的"空则平铺"行为
                tdata = _build_grid_timeline_if_empty(tdata, grid_text, duration_frames, grid_layout, frame_rate)
                timeline_data = json.dumps(tdata, ensure_ascii=False)
        ...
        # 兜底清洗：global_prompt 只应含"全局/正面词"的正文。
        # 万象等外部回填常把"正面词：...全局...第一个镜头..."整段塞进全局提示词，
        # 这里统一先剥离开头的"正面词："标签；若还残留分镜锚点，则只保留第一个镜头之前的部分，
        # 确保"正面词"字样与各分镜内容绝不会泄漏进真正的全局提示词。
        if global_prompt:
            _g = _strip_global_label(global_prompt)
            _anchor = re.search(r"第\s*([0-9]+|[一二三四五六七八九十]+)\s*个?\s*(?:镜头|镜|画面|分镜|shot)", _g, flags=re.I)
            if _anchor:
                _g = _strip_global_label(_g[: _anchor.start()])
            global_prompt = _g.strip()

        log.info(f"[SFLTXDirector] execute RECEIVED global_prompt: {repr(global_prompt)}")
        ...
        try:
            ...
            log.warning("[SFLTXDirector] Could not build motion_guide_data: %s", e)
...
# 1.0 版本 SF-LTXGridDirector 的选项常量（万象模板按此契约生成工作流，必须逐字一致）
_LEGACY_GRID_MODE_OPTIONS = ["2x2 四宫格", "3x2 六宫格", "3x3 九宫格"]
_LEGACY_SHOT_ASPECT_OPTIONS = ["自动 / 保持单格比例", "16:9 横屏", "9:16 竖屏", "1:1 方图"]
_LEGACY_RESIZE_METHOD_MAP = {
    "保持比例": "maintain aspect ratio",
    "拉伸填满": "stretch to fit",
    "留白填充": "pad",
    "裁剪填充": "crop",
    # 英文直通（万一模板里已经是 2.0 写法）
...
_LEGACY_DISPLAY_MODE_MAP = {"秒": "seconds", "帧": "frames", "seconds": "seconds", "frames": "frames"}


class SFGridDirectorLegacy(io.ComfyNode):
    """万象兼容节点：与 1.0 版 SF-LTXGridDirector **完全一致的输入/输出契约**。

    万象软件按 1.0 契约生成最终工作流（见 wxai/*.json）：
    - class_type 必须是带连字符的 `SF-LTXGridDirector`；
    - 字段全是 1.0 命名：storyboard_images / llm_response / grid_mode(字符串布局) /
      shot_aspect / border_crop / parse_mode / resize_method(中文) / display_mode(中文) 等；
    - 输出必须是 1.0 的 7 路布局（索引 5 是帧率，万象的 VHS_VideoCombine 直接引用）。
    本节点 schema 逐字段镜像 1.0，执行时翻译成 2.0 的 SFLTXDirector.execute 调用，
    并从其 8 路输出中丢弃 motion_guide_data(索引5)，还原 1.0 的 7 路输出顺序。
    """

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="SF-LTXGridDirector",
            display_name="SF-LTX 宫格导演台",
            category="SF-WhatDreamsCost",
            description=(
                "LTX 导演台的通用宫格自动版：支持 2x2 四宫格、3x2 六宫格和 3x3 九宫格，"
                "自动接收宫格图和 GPT 分镜文本，同时保留可手动编辑的时间线。（万象 1.0 契约兼容节点）"
            ),
            inputs=[
                io.Model.Input("model", display_name="模型"),
                io.Clip.Input("clip", display_name="文本编码器"),
                io.Image.Input("storyboard_images", display_name="宫格图像", optional=True),
                io.Combo.Input("grid_mode", display_name="宫格模式", options=_LEGACY_GRID_MODE_OPTIONS, default="3x2 六宫格", optional=True),
                io.Combo.Input("shot_aspect", display_name="分镜比例", options=_LEGACY_SHOT_ASPECT_OPTIONS, default="自动 / 保持单格比例", optional=True),
                ...
                io.Combo.Input("resize_method", display_name="图像适配方式",
                               options=["保持比例", "拉伸填满", "留白填充", "裁剪填充"], default="保持比例", optional=True),
    ...
    @classmethod
    def execute(cls, model, clip, storyboard_images=None, grid_mode="3x2 六宫格",
                shot_aspect="自动 / 保持单格比例", border_crop=1.0, llm_response="",
                audio_vae=None, optional_latent=None, global_prompt="",
                duration_frames=120, duration_seconds=5.0, timeline_data="",
                use_custom_audio=False, local_prompts="", segment_lengths="",
                epsilon="0.001", frame_rate=24, display_mode="秒", guide_strength="1.0",
                parse_mode="自动", custom_width=0, custom_height=0,
                resize_method="保持比例", divisible_by=32, img_compression=18) -> io.NodeOutput:
        # --- 1.0 -> 2.0 字段翻译 ---
        try:
            epsilon_f = float(epsilon)
        except (TypeError, ValueError):
            epsilon_f = 0.001
        resize_en = _LEGACY_RESIZE_METHOD_MAP.get(str(resize_method or ""), "maintain aspect ratio")
        display_en = _LEGACY_DISPLAY_MODE_MAP.get(str(display_mode or ""), "seconds")
        duration_frames = max(1, int(duration_frames or 120))
        frame_rate = float(frame_rate or 24)
        # grid_mode(1.0)=布局字符串；grid_layout(2.0) 用同一套选项文本，直接透传
        grid_layout = str(grid_mode or "3x2 六宫格")
        has_grid = storyboard_images is not None

        res = SFLTXDirector.execute(
            model=model, clip=clip,
            start_second=0.0, end_second=float(duration_seconds),
            start_frame=0, end_frame=int(duration_frames),
            duration_seconds=float(duration_seconds), duration_frames=int(duration_frames),
            timeline_data=timeline_data or "",
            local_prompts=local_prompts or "", segment_lengths=segment_lengths or "",
            global_prompt=global_prompt or "",
            guide_strength=str(guide_strength if guide_strength is not None else "1.0"),
            epsilon=epsilon_f, frame_rate=frame_rate, display_mode=display_en,
            custom_width=int(custom_width or 0), custom_height=int(custom_height or 0),
            resize_method=resize_en,
            divisible_by=max(1, int(divisible_by or 32)),
            img_compression=int(img_compression if img_compression is not None else 18),
            audio_vae=audio_vae, optional_latent=optional_latent,
            use_custom_audio=bool(use_custom_audio),
            grid_mode=has_grid, grid_images=storyboard_images, grid_text=llm_response or "",
            grid_layout=grid_layout, grid_aspect=str(shot_aspect or "自动 / 保持单格比例"),
            grid_border_crop=float(border_crop if border_crop is not None else 1.0),
        )
        # 2.0 输出 8 路：(model, cond, latent, audio_latent, guide_data, motion_guide_data, fps, audio)
        # 1.0 契约只有 7 路（索引5=帧率），丢弃 motion_guide_data 还原 1.0 输出顺序
        a = res.args
        return io.NodeOutput(a[0], a[1], a[2], a[3], a[4], a[6], a[7])
