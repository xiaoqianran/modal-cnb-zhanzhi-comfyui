...
" "$title" 2>/dev/null) || disp_w=${#title}
  ...
  printf "%s%*s\n" "$title" "$padding" ""
  echo -e "${CLR_TITLE}${border_line}${CLR_RESET}"
}

label_value() {
  printf "%b%-20s%b %s\n" "$CLR_LABEL" "┃ $1" "$CLR_RESET" "$2"
}
...
if check_command python; then
  ...
  label_value "Python 版本" "$(python --version 2>&1)"
else
...
if check_command nvidia-smi; then
  ...
  echo -e "  ❌ ${CLR_RED}nvidia-smi 未找到，可能未安装 NVIDIA 驱动${CLR_RESET}"
fi

if check_command nvcc; then
  run nvcc --version
else
  echo -e "  ❌ ${CLR_RED}nvcc 未安装，CUDA 工具链不可用${CLR_RESET}"
fi
...
if [[ -d "$CUSTOM_NODE_DIR" ]]; then
  ...
  if [[ "$found_plugins" == false ]]; then
    echo -e "  ❌ ${CLR_RED}custom_nodes 目录为空${CLR_RESET}"
  fi
else
  echo -e "  ❌ ${CLR_RED}custom_nodes 目录不存在${CLR_RESET}"
fi
...
# 检查目录是否存在后再搜索
search_paths=()
for path in /workspace/ComfyUI/models /workspace/comfyui_data/input; do
...
if [[ ${#MODELS[@]} -eq 0 ]]; then
  echo -e "    ❌ ${CLR_RED}未发现模型文件${CLR_RESET}"
else
  maxlen=0
...
if check_command pip; then
  ...
  # 显示前50个包
  echo "$pip_output" | awk '
  BEGIN { count = 0; max_display = 50 }
  NR > 2 && count < max_display { 
    printf "  📦 %-30s %s\n", $1, $2
    count++
  }
  END { 
    if (count >= max_display) {
      print "  ... (显示前" max_display "个包)"
    }
  }'
  
  echo -e "\n  📊 总计: $total_packages 个包"
else
  echo -e "  ❌ ${CLR_RED}pip 未安装${CLR_RESET}"
fi
